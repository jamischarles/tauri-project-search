...
type ResultMatchItem = {
  ...
  data: {
    ...
    path: { text: string },
    submatches: ({ end: number, start: number, match: { text: string } } | "")[]
  }
...
// -> array of objects
export function handleSearchResults(listStr: String, searchPath: string): ReturnVal {
  ...
  const result = listStr.split('\n').map(lineResultStr => {
    if (lineResultStr) {
      ...
      if (lineResult.type === "begin") {
        const relativeFilePath = lineResult.data.path.text.replace(basePath, ".");
      ...
      if (lineResult.type === "match") {
        ...
        lineResult.data.submatches.forEach(inlineResult => {
          ...
          tokenizedText.push(`<span class="inline-match">${lineText.substring(start, end)}</span>`);
          // advance pointer
          pointerPos = end;
        })
